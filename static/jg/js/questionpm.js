    ...
    function ajax_getquestionpm(child_class, query ){
        ...
        $.ajax({
            ...
            $.each( data['q_list'], function(index,value){
                ...
                $tr = $( '<tr>' );
                $td = $('<td>').append( $('<a href=/static/jg/pdf/question_pm/' + value['qfn'] + ' target="_blank">').text( value['qfn']).attr('target','_blank') );
                $tr.append( $td );
                if( value['afn'] != 'ファイルなし' ){
                    $td = $( '<td>' ).append( $('<a href=/static/jg/pdf/question_pm/' + value['afn'] + '>').text( value['afn']).attr('target','_blank') );
                }else{
