$(function(){
    ...
    function ajax_getquestion_period(child_class, query ){
        ...
        $.ajax({
            type:"POST",
            url:"/jg/ajax_getquestion_period/",
            dataType:'json',
            ...
            for( var i = 0 , len=data.length; i<len;++i){
                $p = $( '<p>' ).append( $('<img src=/static/jg/image/question/' + data[i]['q_id'] + '.png alt=' + data[i]['q_id'] + '>').attr('class','question') );
    ...
    function ajax_getquestion_classify(child_class, query ){
        ...
        $.ajax({
            type:"POST",
            url:"/jg/ajax_getquestion_classify/",
            dataType:'json',
            contentType: 'charset=utf-8',
            data: getJsonStr( query ),
        }).done( (data) => {
            $( child_class ).children().remove();
            for( var i = 0 , len=data.length; i<len;++i){
                $p = $( '<p>' ).append( $('<img src=/static/jg/image/question/' + data[i]['q_id'] + '.png alt=' + data[i]['q_id'] + '>') );
                $( child_class ).append( $p );
                $p = $('<p>');
                $p.append( $('<span>').text('答え:') );
                $p.append( $('<span>').attr('class','answer').text( data[i]['q_answer']));
                $( child_class).append( $p );
                console.log( '/static/jg/image/question/' + data[i]['q_id'] + '.png' );
            }
    ...
    function ajax_getperiod(child_class,json){
        ...
        $.ajax({
            type:"POST",
            url:"/jg/mainpage_ajax_getperiod/",
            dataType:'json',
            contentType:'charset=utf-8',
    ...
    $("#period").on('click',function(){
        ...
        $.ajax({
            type:"POST",
            url:"/ip/ajax_period_click/",
            dataType:'json',
            contentType: 'charset=utf-8',
            data: getJsonStr( query ),
        }).done( (data) => {
            $( 'ip_l_field' ).children().remove();
