    ...
    function ajax_getquestionpm(child_class, query ){
        ...
        $.ajax({
            ...
            $.each( data['q_list'], function(index,value){
                ...
                $tr = $( '<tr>' );
                $td = $('<td>').append( $('<a href=/static/jk/pdf/question_pm/' + value['qfn'] + '>').text( value['qfn']).attr('target','_blank') );
                $tr.append( $td );
                if( value['afn'] != 'ファイルなし' ){
                    $td = $( '<td>' ).append( $('<a href=/static/jk/pdf/question_pm/' + value['afn'] + '>').text( value['afn']).attr('target','_blank') );
                }else{
    ...
    function ajax_getquestionjs(child_class, query ){
        ...
        $.ajax({
            ...
            for( var i = 0 , len=data.length; i<len;++i){
                ...
                $tr = $("<tr>");
                $a = $("<a>").attr('href','/static/jk/pdf/question_js/' + data[i]['filename']).attr('target','_blank').text(data[i]['filename']);
                $td = $("<td>").append( $a );
                $tr.append( $td );
